...
As introduced last week, pin numbers are known at compile time and never change, so an array of pins should use the same `constexpr uint8_t` convention as a single pin declaration:
...
- **Wait for the reading to settle** (Example 3): note *when* the raw reading last changed, and only treat it as a real press once it's stayed the same for a minimum stretch of time (e.g. 50 ms) — a small per-button timer, not a burst of samples.
...
void loop() {
  led_chaser();   // one function call sweeps across all 3 LEDs, in array order
}
```
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473317425753399297](https://wokwi.com/projects/473317425753399297)
...
| Inner `i` loop (capture) | Burst-samples the current button 10 times, 5 ms apart |
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473321382592612353](https://wokwi.com/projects/473321382592612353)
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473324549721867265](https://wokwi.com/projects/473324549721867265)
