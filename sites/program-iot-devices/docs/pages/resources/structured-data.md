...
// After: one array of one record type
...
| Section | What it does |
|---|---|
| `struct ButtonLedPair` | Groups one button pin and its paired LED pin into a single record, replacing the two parallel arrays. |
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473326289925248001](https://wokwi.com/projects/473326289925248001)
...
**IoT version — a real sensor filling the struct log.** **Components:** ESP32-S3, potentiometer (stands in for an analog sensor).
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473955613809115137](https://wokwi.com/projects/473955613809115137)

> **Real-world use in industry:** Sensor telemetry packets on an industrial bus (each frame is a fixed-layout record — device ID, value, unit, timestamp — not four unrelated numbers), configuration records in embedded firmware (a `struct` per device profile), and CAN bus / Modbus message definitions, which are essentially structs with a specified byte layout.
...
> **Real-world use in industry:** A device's small settings/configuration table (name → value pairs read from EEPROM or flash at boot), a lookup table mapping an error code to a human-readable message string, and register-name-to-address tables in driver code for talking to a sensor over I²C/SPI.
...
> **Real-world use in industry:** An event log that must always display in time order as new events arrive (alarms, audit trails), a leaderboard that re-ranks itself after every new score without a full re-sort, and any streaming/incremental data pipeline that must stay ordered without re-processing everything already received.
...
void log_reading(String sensor_name, float value, String unit, unsigned long timestamp) {
  if (reading_count >= MAX_READINGS) return;   // overflow guard — same pattern as any fixed array
...
void setup() {
  ...
  if (isnan(humidity) || isnan(temperature)) {   // guard against a failed read — never log it as a real value
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473956741868447745](https://wokwi.com/projects/473956741868447745)
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473957449017751553](https://wokwi.com/projects/473957449017751553)
...
The dictionary lookup itself — `get_threshold("light")` walking `thresholds[]` with a linear search — is unchanged from the syntax example earlier in this page; what's new here is that `lightLevel` comes from a real analog sensor instead of a hardcoded test value, and the sentinel-checked result drives a real LED, not just a `Serial.println()`.
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473958323838595073](https://wokwi.com/projects/473958323838595073)
...
void loop() {
  ...
  if (currentMillis - previousReadMillis >= READ_INTERVAL) {   // non-blocking timing — no delay()
    ...
    float humidity = dht.readHumidity();        // Real sensor read — humidity in %
    float temperature = dht.readTemperature();  // Real sensor read — temperature in °C
    if (isnan(humidity) || isnan(temperature)) {   // guard against a failed read
      Serial.println("Failed to read from DHT22 sensor!");
      return;
    }
...
This is the pattern the rest of the course's builds are graded on: every new value logged into the sorted struct array is immediately checked against a name-keyed threshold table, with the sentinel checked before it's trusted. Nothing here is a new concept — it's the previous three examples (a real sensor feeding a sorted struct log, and a real output driven by a dictionary lookup) used together. The alert LED is driven once per `loop()` pass from both readings together, rather than inside `log_reading()` itself — otherwise the second call (humidity) would silently overwrite whatever the first call (temperature) had just set.
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473959438435232769](https://wokwi.com/projects/473959438435232769)
