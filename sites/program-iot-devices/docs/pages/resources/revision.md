...
When one variable needs to choose between several specific, known values — a mode, a state, a menu choice — `switch`/`case` (also called **Select Case**) expresses that more clearly than a chain of `else if`s. It behaves exactly like an `if`/`else if`/`else` chain that only ever checks equality against one variable; it's a different way of writing the same idea, not a new capability, and it comes with two restrictions an `if` chain doesn't have: each `case` label must be a compile-time constant (a literal or a `const`, never a range like `case 1...5` or a comparison), and the type being switched on must be an integer-like type (`int`, `char`, an `enum`) — never a `String` or `float`.
...
| The requirement says… | The concept it needs |
|---|---|
...
| "…only react the moment it changes, not the whole time it's true" | Edge detection (compare against last pass's reading) |
| "…several related values that must move together" | An array of structs, not parallel arrays |
| "…look this up by name, not by position" | A key-value struct array (dictionary) |
| "…report clearly if the value/sensor wasn't found" | A sentinel return value, checked before use |
| "…always stays in order as new data arrives" | Insertion sort on every insert |
...
| Clause | Concept it names |
|---|---|
| "…only register the drop-off once per press" | Debouncing (or edge detection, if the source is already a clean digital signal) |
| "…only if the door sensor shows the door is actually closed" | Boolean AND combining two conditions |
| "…log the event with its time so the log always displays oldest-to-newest" | A struct record + insertion sort on every insert |
| "…the bike's configured minimum" | A key-value struct array (dictionary) lookup, sentinel-checked |
| "…flash a warning LED without freezing the rest of the system" | Non-blocking `millis()` timing, not `delay()` |
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/474460961380768769](https://wokwi.com/projects/474460961380768769)
...
### Example 2 — Debounced Locker-Select Button
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/474462085306563585](https://wokwi.com/projects/474462085306563585)
...
void loop() {
  ...
  bool confirmPressed = digitalRead(confirmButtonPin) == LOW;
  ...
  bool newConfirmPress = confirmPressed && !lastConfirmPressed;
  ...
  if (confirmActive && millis() - confirmStart >= CONFIRM_DURATION) {
    digitalWrite(confirmLedPin, LOW);
    confirmActive = false;
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/474463588556916737](https://wokwi.com/projects/474463588556916737)
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/473995555083265025](https://wokwi.com/projects/473995555083265025)
...
> Open this Wokwi simulation and write the example code: [https://wokwi.com/projects/474470208794608641](https://wokwi.com/projects/474470208794608641)
...
This single example deliberately touches every concept above: a debounced Return button appends a struct `Checkin` to a sorted log; a dictionary-style battery-threshold table is looked up by name with a checked sentinel to drive a non-blocking low-battery alert; a dock sensor's edge is detected and reported independently; and parameterised functions (`start_alert()`/`update_alert()`, `log_checkin()`, `get_min_battery()`) keep `loop()` itself short.
...
void log_checkin(int batteryLevel) {
  ...
  while (i > 0 && checkins[i - 1].timestamp > checkins[i].timestamp) {
...
void loop() {
  ...
  if ((millis() - debounceStart >= DEBOUNCE_TIME) && (reading != buttonState)) {
    ...
    if (buttonState == LOW) {
      bool docked = digitalRead(dockSensorPin) == HIGH;
      ...
      if (!docked) {
        ...
        int batteryLevel = map(analogRead(batteryPin), 0, 4095, 0, 100);

        log_checkin(batteryLevel);

        int minBattery = get_min_battery("standard");

        if (minBattery >= 0 && batteryLevel < minBattery) {
...
| `lastButtonReading` / `buttonState` / `debounceStart` / `DEBOUNCE_TIME` | The debounce state for the Return button — identical pattern to Examples 2 and 6. |
...
| `log_checkin()` — append & guard | `if (checkin_count >= MAX_CHECKINS) return;` then appends `{millis(), batteryLevel}` — the same fixed-array overflow guard used throughout the course. |
...
| `loop()` — dock sensor block | `dockNow != lastDockState` fires only on the pass the dock reading actually changes (edge detection), reporting docked/undocked once per transition instead of on every pass it holds that value. |
