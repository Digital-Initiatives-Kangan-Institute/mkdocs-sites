# Combining Multiple Sensors Activity
...
!!! abstract "Instructions"
    **Scenario:** A workshop indicator panel needs a red, a green, and a blue LED blinking at different speeds depending on the situation, without duplicating the same `digitalWrite`/`delay` code for each one.

    **Components:** ESP32-S3 development board, red LED, green LED, blue LED, 3× 220 Ω resistor, jumper wires.
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472678434137535489](https://wokwi.com/projects/472678434137535489)
...
!!! abstract "Instructions"
    **Scenario:** A stockroom needs a single LED alert that responds to two inputs — a PIR sensor and a manual button — but motion should always take priority if both happen to trigger in the same instant. The status message must not flood the Serial monitor with a duplicate line on every single pass of `loop()`.

    **Components:** ESP32-S3 development board, PIR sensor, push button (`INPUT_PULLUP`), LED, 220 Ω resistor, jumper wires.
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472678982127757313](https://wokwi.com/projects/472678982127757313)
...
!!! abstract "Instructions"
    ...
    **Components:** ESP32-S3 development board, PIR sensor, push button (`INPUT_PULLUP`) — after-hours toggle, passive buzzer, jumper wires.
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472681863784370177](https://wokwi.com/projects/472681863784370177)
...
!!! abstract "Instructions"
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472682245567918081](https://wokwi.com/projects/472682245567918081)
...
!!! abstract "Instructions"
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472683682988874753](https://wokwi.com/projects/472683682988874753)
    ...
    **Check yourself:**
    ...
    - The test stops automatically once `testDuration` has elapsed, turning all three LEDs off and clearing `testRunning`
    - `loop()` contains no `delay()` calls anywhere, and all `millis()`-based timing variables are `unsigned long`
...
!!! abstract "Instructions"
    ...
    **Components:** ESP32-S3 development board, PIR sensor, push button (`INPUT_PULLUP`), LED, passive buzzer, SSD1306 OLED display (128×64, I²C), 220 Ω resistor, jumper wires.
    ...
    **Open this Wokwi simulation:** [https://wokwi.com/projects/472684827957585921](https://wokwi.com/projects/472684827957585921)
...
??? hint "Answer — Click to expand"
    The second branch can never be reached in the way intended — once `motionDetected` is false (the only way to reach the `else if`), `motionDetected || buttonPressed` just collapses to checking `buttonPressed` alone. The condition should simply be `buttonPressed`.
...
??? hint "Answer — Click to expand"
    `start_buzzer()` never records `buzzerStartTime = millis();` — without it, `buzzerStartTime` stays at its old value (or 0), so `update_buzzer()`'s timing check is meaningless.
...
??? hint "Answer — Click to expand"
    `previousMillis` is declared (and reset to `0`) **inside** `loop()`, so it loses its value the instant the function returns. Every single pass, `currentMillis - previousMillis` is computed against a fresh `0`, which is always `>= interval` — so `blink_led()` fires on every pass instead of once every 500 ms. `previousMillis` needs to keep its value between calls, either by declaring it `static` inside `loop()` or as a variable outside `loop()` entirely.
...
??? hint "Answer — Click to expand"
    There's no `return false;` for the case where the PIR sensor reads `LOW` — a function declared to return `bool` that doesn't return on every path gives an unreliable result when no motion is detected.
...
??? hint "Answer — Click to expand"
    `|` is the bitwise OR operator, not the logical OR. It happens to evaluate correctly here because both sides are already `bool`, but it's the wrong tool and can silently misbehave with non-boolean values — it should be `||`.
...
??? hint "Answer — Click to expand"
    `setup()` never calls `display.begin(SSD1306_SWITCHCAPVCC, 0x3C)` — without initialising the SSD1306 driver chip first, the display object isn't ready to receive commands, so `update_display()` will have no visible effect (or the sketch may hang/crash on some boards).
